...
def _try_write_json(data: Any, out_path: Path) -> None:
    ...
    try:
        ...
        if target_path.exists():
            ...
            if isinstance(data, list):
                return filter_ptns(data, provinsi=provinsi, kota=kota)
    except Exception as e:
        logger.error("Fallback reading %s failed: %s", out_path, e)
    return []


ALLOWED_UPSTREAM_PREFIX = "https://snpmb.id/"
...
async def fetch_prodi(
    ...
    if not url.startswith(ALLOWED_UPSTREAM_PREFIX):
        logger.error("Blocked attempt to fetch unauthorized upstream URL: %s", url)
        return []
        ...
        async with (
            ...
            if resp.status == 200:
                ...
                if isinstance(raw_data, list):
                    cleaned = [clean_prodi(item, is_snbp) for item in raw_data if isinstance(item, dict)]
    ...
    try:
        ...
        async with (
            ...
            if resp.status == 200:
                raw_data = await resp.json(content_type=None)
                if isinstance(raw_data, list):
                    cleaned = [clean_ptn(item) for item in raw_data if isinstance(item, dict)]
